...
include: "/commerce_first_party/data_portal/extended_views/fp_inbound_metrics.view.lkml"



explore: develop_1p_sku_dos {
  hidden: yes
  ...
  join: fp_orders {
    ...
    sql_on: ${fp_dimension_fields_periods.ord_closed_dt}    = ${fp_orders.date_day}
        AND ${fp_dimension_fields_periods.sit_site_id}      = ${fp_orders.sit_site_id}
        AND ${fp_dimension_fields_periods.ite_item_sap_sku} = ${fp_orders.ite_item_sap_sku}
        AND ${fp_dimension_fields_periods.ite_item_id}      = ${fp_orders.ite_item_id}
        AND ${fp_dimension_fields_periods.period_type}      = ${fp_orders.period_type}

        ;;
  }

  join: fp_inventory_metrics {
    from: fp_inventory_metrics
    type: left_outer
    relationship: one_to_one
    sql_on: ${fp_dimension_fields_periods.ord_closed_dt}    = ${fp_inventory_metrics.ord_closed_date}
        AND ${fp_dimension_fields_periods.sit_site_id}      = ${fp_inventory_metrics.sit_site_id}
        AND ${fp_dimension_fields_periods.ite_item_sap_sku} = ${fp_inventory_metrics.ite_item_sap_sku}
        AND ${fp_dimension_fields_periods.ite_item_id}      = ${fp_inventory_metrics.ite_item_id}
        AND ${fp_dimension_fields_periods.period_type}      = ${fp_inventory_metrics.period_type}

        ;;
  }

  join: fp_ue_managerial {
    from: fp_ue_managerial
    type: left_outer
    relationship: one_to_one
    sql_on: ${fp_dimension_fields_periods.ord_closed_dt}    = ${fp_ue_managerial.dia_final}
        AND ${fp_dimension_fields_periods.sit_site_id}      = ${fp_ue_managerial.sit_site_id}
        AND ${fp_dimension_fields_periods.ite_item_sap_sku} = ${fp_ue_managerial.ite_item_sku_id}
        AND ${fp_dimension_fields_periods.ite_item_id}      = ${fp_ue_managerial.ite_item_id}
        AND ${fp_dimension_fields_periods.period_type}      = ${fp_ue_managerial.period_type}
        ;;
  }
...
#AND ${fp_dimension_fields_periods.ite_item_id} = COALESCE(${fp_purchase_order.ite_item_id},0)



  join: fp_oos_historico {
    from: fp_oos_historico
    type: left_outer
    relationship: one_to_one
    sql_on: ${fp_dimension_fields_periods.ord_closed_dt}    = ${fp_oos_historico.tim_day}
        AND ${fp_dimension_fields_periods.sit_site_id}      = ${fp_oos_historico.sit_site_id}
        AND ${fp_dimension_fields_periods.ite_item_sap_sku} = ${fp_oos_historico.ite_item_sap_sku}
        AND ${fp_dimension_fields_periods.period_type}      = ${fp_oos_historico.period_type}
        AND ${fp_dimension_fields_periods.ite_item_id_join} = ${fp_oos_historico.ite_item_id_join}
        ;;
  }
  ...
  #AND ${fp_dimension_fields_periods.ite_item_id}      = COALESCE(${fp_lk_selection_funnel_sku.ite_item_id},0)
    ...
    join: fp_share_metrics {
     ...
     sql_on: ${fp_dimension_fields_periods.ord_closed_dt}    = ${fp_share_metrics.ord_closed_dt}
         ...
         AND ${fp_dimension_fields_periods.vertical_domain}  = ${fp_share_metrics.vertical_domain}
         AND ${fp_dimension_fields_periods.dom_domain_agg2}  = ${fp_share_metrics.dom_domain_agg2}
         AND ${fp_dimension_fields_periods.dom_domain_agg1}  = ${fp_share_metrics.dom_domain_agg1}
  ...
  join: fp_planes {
    ...
    sql_on: ${fp_dimension_fields_periods.ord_closed_dt}    = ${fp_planes.ord_closed_dt}
        ...
        AND ${fp_dimension_fields_periods.ite_item_brand} = ${fp_planes.ite_item_brand}
        AND ${fp_dimension_fields_periods.ite_item_sap_sku} IS NULL
        AND ${fp_dimension_fields_periods.ite_item_id}      IS NULL
  ...
  join: fp_ue_consolidated {
    from: fp_ue_consolidated
    type: left_outer
    relationship: one_to_one
    sql_on: ${fp_dimension_fields_periods.ord_closed_dt}    = ${fp_ue_consolidated.ord_closed_dt}
        AND ${fp_dimension_fields_periods.sit_site_id}      = ${fp_ue_consolidated.sit_site_id}
        AND ${fp_dimension_fields_periods.ite_item_sap_sku} = ${fp_ue_consolidated.ite_item_sap_sku}
        AND ${fp_dimension_fields_periods.period_type}      = ${fp_ue_consolidated.period_type}
        AND ${fp_dimension_fields_periods.ite_item_id}      = ${fp_ue_consolidated.ite_item_id}
        ;;
  }

  join: fp_bpc_favorability {
    from: fp_bpc_favorability
    type: left_outer
    relationship: one_to_one
    sql_on: ${fp_dimension_fields_periods.ord_closed_dt}    = ${fp_bpc_favorability.ord_closed_dt}
        AND ${fp_dimension_fields_periods.sit_site_id}      = ${fp_bpc_favorability.sit_site_id}
        AND ${fp_dimension_fields_periods.ite_item_sap_sku} = ${fp_bpc_favorability.ite_item_sap_sku}
        AND ${fp_dimension_fields_periods.ite_item_id}      = ${fp_bpc_favorability.ite_item_id}
        AND ${fp_dimension_fields_periods.period_type}      = ${fp_bpc_favorability.period_type}
        ;;
  }

  join: fp_inbound_metrics {
    from: fp_inbound_metrics
    type: left_outer
    relationship: one_to_one
    sql_on: ${fp_dimension_fields_periods.ord_closed_dt}    = ${fp_inbound_metrics.ord_closed_dt}
        AND ${fp_dimension_fields_periods.sit_site_id}      = ${fp_inbound_metrics.sit_site_id}
        AND ${fp_dimension_fields_periods.ite_item_sap_sku} = ${fp_inbound_metrics.ite_item_sap_sku}
        AND ${fp_dimension_fields_periods.ite_item_id_join} = ${fp_inbound_metrics.ite_item_id_join}
        AND ${fp_dimension_fields_periods.period_type}      = ${fp_inbound_metrics.period_type}
        ;;
  }
